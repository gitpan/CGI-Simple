...
BEGIN { $SIG{__DIE__} = sub { croak "Undefined Method : @_\n" } }
...
sub import {
    ...
    for my $arg(@args) {
        ...
        if ( $arg =~ m/^:[-\w]+$/ ) {
            if (exists $EXPORT_TAGS{$arg}) {
                ...
                for my $tag (@tags) {
                    my @expanded = exists $EXPORT_TAGS{$tag} ? @{$EXPORT_TAGS{$tag}} : ($tag);
                    $exports{$_}++ for @expanded;
                }
            }
            else {
                croak "No '$arg' tag set available for export from CGI::Simple::Standard!\n";
            }
        }
    }
    ...
    for (@exports) {
        croak "'$_' is not an available export method from CGI::Simple::Standard!\n"
            unless exists $valid_exports{$_};
    }
    no strict 'refs';
    if ( exists $pragmas{'-autoload'} ) {
        # hack symbol table to export our AUTOLOAD sub
        *{"${package}::AUTOLOAD"} = sub{ my($caller, $sub) = $AUTOLOAD =~ m/(.*)::(\w+)$/; &CGI::Simple::Standard::loader($caller,$sub,@_) };
    ...
    for my $i ( 0 .. $#exports ) {
        *{"${package}::$exports[$i]"} = sub { my $caller = caller; &CGI::Simple::Standard::loader($caller,$exports[$i],@_) }
    }
}
