...
sub new {
    ...
    rearrange( ['NAME',['VALUE','VALUES'],'PATH','DOMAIN','SECURE','EXPIRES'], @params );
...
sub expires {
    my( $self, $expires ) = @_;
    $self->{'expires'} = CGI::Simple::Util::expires($expires,'cookie') if defined $expires;
  return $self->{'expires'};
}
