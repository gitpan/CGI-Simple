...
sub escape {
  ...
  return undef unless defined $toencode;
    if ( $UTIL->{'ebcdic'} ) {
      $toencode =~ s/([^a-zA-Z0-9_.-])/uc sprintf "%%%02x", $UTIL->{'e2a'}->[ord $1]/eg;
    } else {
...
BEGIN {
    ...
    sub charset {
        ...
        $self->{'charset'} ||= 'ISO-8859-1';  # set to the safe ISO-8859-1 if not defined
      return $self->{'charset'};
    }
    ...
    sub os {
        ...
        $OS = $self->{'os'};
        unless ( $OS ) {
            unless ($OS = $^O) { require Config; $OS = $Config::Config{'osname'} }
            if ($OS=~/Win/i) {
                $OS = 'WINDOWS';
            } elsif ($OS=~/vms/i) {
                $OS = 'VMS';
