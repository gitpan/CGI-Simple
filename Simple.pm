...
sub _reset_globals { _use_cgi_pm_global_settings() } # used in CGI.pm .t files
...
BEGIN {
    ...
    if ( "\t" ne "\011" ) {
        ...
        $@ && croak "Your server is using not using ASCII, you must install CGI::Simple::Util, error: $@";
...
sub _initialize {
    ...
    # elsif ( (ref $init) =~ m/GLOB/i ) { # initialize from a file
        ...
        require Data::Dumper;
        my $VAR1; # avoid problems with strict when Data::Dumper returns $VAR1
  ...
  return () unless defined $data;
    ...
    for my $pair(@pairs) {
      ...
      next unless defined $param;
        ...
        $self->_add_param( $self->url_decode($param), $self->url_decode($value) );
    }
}
...
sub _add_param {
  ...
  return () unless defined $param and defined $value;
    ...
    for my $value (@values) {
      next if $value eq '' and $self->{'.globals'}->{'NO_UNDEF_PARAMS'};
...
sub _parse_multipart {
    ...
    $boundary = '--'.$boundary unless $ENV{'HTTP_USER_AGENT'} =~ m/MSIE\s+3\.0[12];\s*Mac/i;
...
READ:
    ...
    BOUNDARY:
        ...
        while ( $data =~ m/^$boundary$CRLF/ ) {
          ...
          next READ unless $data =~ m/^([\x20-\x7E\x80-\xFF\x09$CRLF]+?$CRLF$CRLF)/o;
            my $header = $1;
          ...
          next READ unless $data =~ s/^\Q$header\E(.*?)$CRLF(?=$boundary)//s;
...
sub param {
    ...
    ( $param, undef, @p ) = @p if  $param =~ m/^-name$/i; # undef represents -value token
    $self->_add_param( $param, ( ref $p[0] eq 'ARRAY' ? $p[0] : [@p] ), 'overwrite' );
  return wantarray ? @{$self->{$param}} : $self->{$param}->[0];
}
...
sub keywords {
    my( $self, @values ) = @_;
    $self->{'keywords'} = ref $values[0] eq 'ARRAY' ? $values[0] : [@values] if @values;
...
sub STORE    { my($q,$p,$v)=@_;$q->param($p,split$q->{'.sep'}, $v) }
sub FETCH    { my($q,$p)=@_; ref $q->{$p} eq "ARRAY" ? join $q->{'.sep'}, @{$q->{$p}} : $q->{$p} }
...
sub append {
  ...
  return () unless defined $param;
    ...
    ( $param, undef, @p ) = @p if  $param =~ m/^-name$/i; # undef represents -value token
    $self->_add_param( $param, ( (defined $p[0] and ref $p[0]) ? $p[0] : [@p] ) );
...
sub delete {
    my ( $self, $param ) = @_;
  return () unless defined $param;
    $param = $param =~ m/^-name$/i ? shift : $param; # allow delete(-name=>'foo') syntax
  return undef unless defined $self->{$param};
    delete $self->{$param};
    delete $self->{'.fieldnames'}->{$param};
    $self->{'.parameters'} = [ grep { $_ ne $param } @{$self->{'.parameters'}} ];
}
...
sub upload {
    ...
    unless ($filename) {
        $self->cgi_error("No filename submitted for upload to $writefile") if $writefile;
    ...
    for my $param ($self->param) {
        for my $value ($self->param($param)) {
          next unless defined $value;
            push @pairs, $self->url_encode($param).'='.$self->url_encode($value);
        }
    }
  return join $self->{'.globals'}->{'USE_PARAM_SEMICOLONS'} ? ';' : '&', @pairs;
}
...
sub save {
    ...
    for my $param($self->param) {
        for my $value ( $self->param($param) ) {;
            print $fh $self->url_encode($param), '=', $self->url_encode($value), "\n";
        }
    }
    print $fh "=\n";
}
...
sub print { shift; CORE::print(@_) } # print to standard output (for overriding in mod_perl)
...
sub cookie {
    ...
    CGI::Simple::Util::rearrange( ['NAME',['VALUE','VALUES'],'PATH','DOMAIN',
        'SECURE','EXPIRES'], @params );
    # retrieve the value of the cookie, if no value is supplied
    unless ( defined($value) ) {
        $self->{'.cookies'} = CGI::Simple::Cookie->fetch unless $self->{'.cookies'};
      return () unless $self->{'.cookies'};
        # if no name is supplied, then retrieve the names of all our cookies.
      return keys %{$self->{'.cookies'}} unless $name;
        # return the value of the cookie
      return exists $self->{'.cookies'}->{$name} ? $self->{'.cookies'}->{$name}->value : ();
    }
  ...
  return undef if $self->{'.header_printed'}++ and $self->{'.globals'}->{'HEADERS_ONCE'};
    ...
    if ($cookie) {
        ...
        for my $cookie (@cookie) {
            my $cs = ref $cookie eq 'CGI::Simple::Cookie' ? $cookie->as_string : $cookie;
    ...
    $expires = 'now' if $self->no_cache;  # encourage no caching via expires now
    push @header, "Expires: " . CGI::Simple::Util::expires($expires,'http') if $expires;
    push @header, "Date: " . CGI::Simple::Util::expires(0,'http') if defined $expires || $cookie || $nph;
...
sub redirect {
    ...
    unshift @o, '-Status'=>'302 Moved', '-Location'=>$url, '-nph'=>$nph;
...
sub multipart_init {
    ...
    my $warning = "WARNING: YOUR BROWSER DOESN'T SUPPORT THIS SERVER-PUSH TECHNOLOGY.";
    $self->{'.separator'} = "$CRLF--$boundary$CRLF";
    $self->{'.final_separator'} = "$CRLF--$boundary--$CRLF$warning$CRLF";
    my $type = 'multipart/x-mixed-replace;boundary="' . $boundary . '"';
  return $self->header(    -nph=>1, -type=>$type,    map{split"=",$_,2}@other)
...
sub Dump {
    ...
    (my $dump = Data::Dumper::Dumper(@_)) =~ tr/\000/0/;  # remove null bytes cgi-lib.pl
...
sub cgi_error {
    ...
    if ( $err ) {
         ...
         $self->{'.globals'}->{'FATAL'} == 1 ? croak $err :
            $self->{'.globals'}->{'FATAL'} == 0 ?  carp $err : return $err;
    }
...
sub ReadParse {
    ...
    my $pkg = caller();
    no strict 'refs';
    *in = @_ ? $_[0] : *{"${pkg}::in"}; # set *in to passed glob or export *in
    %in = $q->Vars;
    $in{'CGI'} = $q;
  return scalar %in;
}

sub SplitParam { &_shift_if_ref; defined $_[0] && (wantarray ? split "\0", $_[0] : (split "\0", $_[0])[0]) }

sub MethGet { request_method() eq 'GET' }

sub MethPost { request_method() eq 'POST' }

sub MyBaseUrl { local $^W=0; 'http://'.server_name().(server_port()!=80?':'.server_port():'').script_name() }

sub MyURL  { MyBaseUrl() }

sub MyFullUrl { local $^W=0; MyBaseUrl().$ENV{'PATH_INFO'}.($ENV{'QUERY_STRING'}?"?$ENV{'QUERY_STRING'}":'') }

sub PrintHeader { ref $_[0] ? $_[0]->header() : "Content-Type: text/html\n\n" }

sub HtmlTop { &_shift_if_ref; "<html>\n<head>\n<title>$_[0]</title>\n</head>\n<body>\n<h1>$_[0]</h1>\n" }
...
sub CgiError {
    &_shift_if_ref;
    @_ = @_ ? @_ : ("Error: script ".MyFullUrl()." encountered fatal error\n");
...
sub nph { $_[0]->{'.globals'}->{'NPH'} = $_[1] if defined $_[1]; return $_[0]->{'.globals'}->{'NPH'} }

sub all_parameters { $_[0]->param }

sub charset { require CGI::Simple::Util; $CGI::Simple::Util::UTIL->charset($_[1]) }
...
sub user_name        { $ENV{'HTTP_FROM'} || $ENV{'REMOTE_IDENT'} || $ENV{'REMOTE_USER'} }

sub user_agent {
    my ( $self, $match ) = @_;
  return $match? $ENV{'HTTP_USER_AGENT'} =~ /\Q$match\E/i : $ENV{'HTTP_USER_AGENT'};
}
...
sub path_info {
    ...
    if ( defined $info ) {
        ...
        $self->{'.path_info'} = defined($ENV{'PATH_INFO'}) ? $ENV{'PATH_INFO'} : '';
...
sub self_url {
    my( $self, @params ) = @_;
    return $self->url( '-path_info'=>1, '-query'=>1, '-full'=>1, @params );
}
...
Address bug reports and comments to: jfreeman@tassie.net.au.  When sending
